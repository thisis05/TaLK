...
def get_raw_data_Photo(root, train_ratio=0.6, val_ratio=0.2,  seed=0):

    csv_path = os.path.join(root, 'Photo/Photo.csv')
    df = pd.read_csv(csv_path)
    text = list(df['text'])
    data = torch.load(os.path.join(root, 'Photo/Photo_pyg.pt'), weights_only=False)
